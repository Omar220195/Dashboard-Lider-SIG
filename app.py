...
.filter-bar{background:white;border:0.5px solid rgba(0,0,0,0.08);border-radius:8px;padding:12px 16px;margin-bottom:16px}
.filter-title{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#7a7a74;margin-bottom:10px}
...
div[data-testid="stDateInput"] label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.4px;color:#7a7a74}
</style>
...
def process_df(file_bytes):
    ...
    if "Devolver_Reconocedor"     not in df.columns: df["Devolver_Reconocedor"]     = ""
    if "Observaciones_Devolucion" not in df.columns: df["Observaciones_Devolucion"] = ""
    return df

def semana_label(d):
    """Lunes de la semana como etiqueta."""
    if pd.isnull(d): return None
    lunes = d - timedelta(days=d.weekday())
    return lunes.strftime("Sem %d/%m")
...
with st.sidebar:
    ...
    uploaded = st.file_uploader(
        "Cargar Excel del aplicativo",
        type=["xlsx", "xls"],
        help="Descarga el Excel desde LíderSIG y súbelo aquí.",
    )
    if uploaded:
        st.success(f"✓ {uploaded.name}")
    st.markdown("---")
    st.markdown("<small style='color:#7a7a74'>GEO Proyecciones · Catastral V2</small>",
                unsafe_allow_html=True)
...
if not uploaded:
    st.info("👈 Carga el Excel del aplicativo en el panel izquierdo para ver el dashboard.")
    st.stop()

df = process_df(uploaded.read())

# Rango global de fechas del dataset
fecha_min = min(
    df["Fecha_Asig_Coord"].min(),
    df["Fecha_Asig_Editor"].min(),
) 
fecha_max = max(
    df["Fecha_Asig_Editor"].max(),
    df["Fecha_Terminado"].max() if df["Fecha_Terminado"].notna().any() else df["Fecha_Asig_Editor"].max(),
)
...
# Tabs para separar filtros generales vs. por fecha
...
# Construcción del timeline semanal
def build_weekly(df_in):
    ...
    for col_fecha, etiqueta in [
        ...
        sub["Semana"] = sub[col_fecha].apply(
            lambda d: (d - timedelta(days=d.weekday())).strftime("Sem %d/%m/%y") if pd.notna(d) else None
        )
    ...
    out = pd.concat(rows).sort_values("Semana_ord")
    return out
...
if not weekly.empty:
    ...
    with c1:
        fig_w = px.bar(
            weekly, x="Semana", y="Espacios", color="Evento",
            barmode="group",
    ...
    with c2:
        # Acumulado de terminados en el tiempo
        term_daily = (
            dff[dff["Fecha_Terminado"].notna()]
            .groupby("Fecha_Terminado")
            .size()
...
with c1:
    ...
    st.plotly_chart(fig, use_container_width=True)
with c2:
    fig2 = px.bar(coord_tot, x="Coordinador", y="Capturas", title="Capturas por coordinador",
...
st.dataframe(coord_pivot[disp].sort_values("Total", ascending=False), use_container_width=True, hide_index=True)
...
st.dataframe(ed_pivot[disp_e].sort_values("Total", ascending=False), use_container_width=True, hide_index=True)
